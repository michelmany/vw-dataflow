        ...
        <div data-testid="user-drawer" role="dialog" aria-modal="true">
          ...
          >
            ...
            <input 
              ...
              required
            />
            <select name="role" data-testid="form-role" defaultValue={userToEdit?.role || ''}>
              <option value="">Select Role</option>
              <option value="admin">Admin</option>
              <option value="editor">Editor</option>
              <option value="viewer">Viewer</option>
            </select>
            <select name="status" data-testid="form-status" defaultValue={userToEdit?.status || ''}>
              <option value="">Select Status</option>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
            <select name="team" data-testid="form-team" defaultValue={userToEdit?.team || ''}>
            ...
            <button type="button" data-testid="cancel-form" onClick={onDrawerClose}>
              Cancel
            </button>
          </form>
        </div>
  ...
  const handleUserSubmit = async (formData: any) => {
    ...
    try {
      if (userToEdit) {
        ...
        const response = await fetch(`http://localhost:3001/api/users/${userToEdit.id}`, {
          method: 'PUT',
...
describe('User CRUD Integration Tests', () => {
  ...
  describe('Read Operations', () => {
    it('displays list of users on page load', async () => {
      ...
      expect(screen.getByTestId('user-email-1')).toHaveTextContent('john.doe@example.com')
  ...
  describe('Create Operations', () => {
    it('opens add user form when add button is clicked', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getAllByText('Add User')).toHaveLength(2) // Button and heading
      })
    })
  ...
  describe('Update Operations', () => {
    it('opens edit form with pre-filled data', async () => {
      ...
      expect(screen.getByTestId('form-email')).toHaveValue('john.doe@example.com')
    })
  ...
  describe('Error Handling', () => {
    it('handles API errors gracefully during user creation', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('user-drawer')).toBeInTheDocument()
      }, { timeout: 3000 })
    })
  })
